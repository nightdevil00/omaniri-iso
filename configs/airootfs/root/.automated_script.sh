...
install_base_system() {
  ...
  # These patches target archinstall <= v3.0.7; remove after upstream ships a fix.
  ARCHINSTALL_INSTALLER=$(find /usr/lib/python3*/site-packages/archinstall/lib/installer.py -print -quit 2>/dev/null || true)
  if [[ -n "$ARCHINSTALL_INSTALLER" ]]; then
    sed -i \
